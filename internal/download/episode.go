...
func Episode(client *api.Client, baseContentID string, info *api.EpisodeInfo, audioLangs, subsLangs []string, videoQuality, audioQuality *string) error {
	...
	defer func() {
		...
		for id, sToken := range activeStreams {
			if _, err := client.DeleteStream(id, sToken); err != nil {
				fmt.Printf("\nFailed to remove stream %s: %v\n", id, err)
			}
		}
	}()

	firstEpisode, err := client.GetEpisode(versions[0].contentId)
	if err != nil {
	...
	for _, locale := range subsLangs {
		...
		file, err := media.DownloadSubs(firstEpisode.Subtitles[locale].URL)
		if err != nil {
	...
	for i, version := range versions {
		...
		if i > 0 {
			episode, err = client.GetEpisode(version.contentId)
			if err != nil {
		...
		manifestData, err := client.FetchManifest(episode.ManifestURL)
		if err != nil {
		...
		keys, err := drm.GetLicense(client, *pssh, version.contentId, episode.Token)
		if err != nil {
		...
		audioFile, err := media.DownloadParts(audioBaseUrl, audioRepresentationId, audioSet, keys)
		if err != nil {
		...
		if i == 0 {
			...
			videoFile, err = media.DownloadParts(baseUrl, representationId, videoSet, keys)
			if err != nil {
				return fmt.Errorf("downloading video: %w", err)
			}
		}

		if success, err := client.DeleteStream(version.contentId, episode.Token); err != nil {
